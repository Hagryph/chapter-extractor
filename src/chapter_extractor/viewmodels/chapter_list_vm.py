...
class ChapterListViewModel(QObject):
    """Orchestrates source model + filter proxy + selection state.

    Views consume ``proxy_model`` (filtered/sorted view) and listen to
    ``selection_changed`` to drive the reader pane. ``Signal(object)`` is
    Qt's idiomatic typing for arbitrary Python references — here, an
    ``Optional[ChapterSummary]``.
    """

    selection_changed = Signal(object)   # ChapterSummary | None
    chapters_loaded = Signal(int)         # row count after refresh
    chapter_added = Signal(int)           # chapter_id
    chapter_soft_deleted = Signal(int)    # chapter_id

    def __init__(
        self, repo: IChapterRepository, parent: QObject | None = None
    ) -> None:
        super().__init__(parent)
        self._repo = repo
        # Parent the models on self so Qt's parent-child mechanism cleans
        # them up automatically when the VM is deleted.
    ...
    def refresh(self) -> None:
        ...
        if (
            ...
            and self._source.find_row_by_id(self._selected_id) == -1
        ):
