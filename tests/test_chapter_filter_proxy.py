...
class TestChapterFilterProxy:
    ...
    def test_filter_by_chapter_number(self, qt_app) -> None:
        ...
        proxy.setSourceModel(src)
        proxy.set_needle("51")
        # 51 is a substring of "51" and "510".
        assert proxy.rowCount() == 2
    ...
    def test_set_needle_invalidates_filter_dynamically(self, qt_app) -> None:
        # Verifies set_needle triggers re-evaluation: changing the needle
        # without re-setting the source model still updates row count.
        del qt_app
        src = ChapterListModel([_make(1, 1, "alpha"), _make(2, 2, "beta")])
        proxy = ChapterFilterProxy()
        proxy.setSourceModel(src)
        proxy.set_needle("alpha")
        assert proxy.rowCount() == 1
        proxy.set_needle("beta")
        assert proxy.rowCount() == 1
        proxy.set_needle("gamma")
