...
class TestProjectListViewModel:
    ...
    def test_create_persists_and_emits(self, qt_app, qtbot, registry, tmp_path: Path) -> None:
        del qt_app
        vm = ProjectListViewModel(registry)
    ...
    def test_open_emits_project(self, qt_app, qtbot, registry, tmp_path: Path) -> None:
        del qt_app
        vm = ProjectListViewModel(registry)
    ...
    def test_toggle_pin(self, qt_app, registry, tmp_path: Path) -> None:
        ...
        vm = ProjectListViewModel(registry)
        vm.create("X", tmp_path)
        original = vm.projects[0]
        vm.toggle_pin(original)
